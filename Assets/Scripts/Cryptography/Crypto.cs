...
namespace MRK.Cryptography
{
    public class Crypto
    {
        ...
        tvcSH|cz
        */
        private const string RawSalt = @"HvpHtNH0@aC|C1Epn~e02KLAm}ewKK\pfVKCmWfT";
        private const string RawSaltDecipher = "[pj}szLwpkV|z{Lw~tzq";
        private const string RawDecipher = "tvcSH|cz";

        private static string _cookedSalt;

        public static string CookedSalt
        {
            get
            {
                return _cookedSalt;
        ...
        public static void CookSalt()
        {
            ...
            byte[] rawDecipher = GetUTF8Mem(RawDecipher);
            Cook(rawDecipher, m); //0x31

            byte[] rawSaltDecipher = GetUTF8Mem(RawSaltDecipher);
            Cook(rawSaltDecipher, rawDecipher);

            byte[] salt = GetUTF8Mem(RawSalt);
            Cook(salt, rawSaltDecipher);

            _cookedSalt = Encoding.UTF8.GetString(salt);
        }
        ...
        public static string Hash(string raw)
        {
            ...
            {
                //apply salt
                byte[] inputBytes = Encoding.ASCII.GetBytes(raw + _cookedSalt);
